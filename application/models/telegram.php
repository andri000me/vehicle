...
class __Module_Telegram_Sender extends CI_Model{
	...
	public function _url($with_method = FALSE){	
	...
	function RequestWebhook($method, $parameters) {
		if (!is_string($method)) {
			...
			//Added by SFS (write log file)
			log_message("error","Method name must be a string\n");
			return false;
		}
		...
		if (!$parameters) {
			...
			//Added by SFS (write log file)
			log_message("error","Parameters must be an array\n");
			return false;
		}
	...
	function exec_curl_request($handle) {
		...
		if ($response === false) {
			...
			//Added by SFS (write log file)
			log_message("error","Curl returned error $errno: $error\n");
			curl_close($handle);
			return false;
		}
		...
		// do not wat to DDOS server if something goes wrong
			...
			//Added by SFS (write log file)
			log_message("error","Request has failed with error {$response['error_code']}: {$response['description']}\n");
			if ($http_code == 401) {
			...
			if (isset($response['description'])) {
				...
				//Added by SFS (write log file)
				log_message("error","Request was successfull: {$response['description']}\n");
			}
